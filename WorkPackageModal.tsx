...
const WorkPackageCard: React.FC<Props> = ({ wp, tasks, onAddTask, onDeleteTask, onEditTask, onDeleteWP, onMoveTask, onRenameWP, allTasks }) => {
  ...
  return (
    ...
    >
        ...
        {tasks.map((task) => (
          <div 
            ...
            className={`
              ...
              ${task.isOTD 
                ? 'border-blue-200 bg-blue-50/80 shadow-blue-100/50 hover:border-blue-400' 
          ...
          >
            {task.isOTD && (
              <div className="absolute -top-2 right-8 bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm z-30 flex items-center justify-center border border-white whitespace-nowrap min-w-[32px]">
                OTD
              </div>
            )}
            {task.isCritical && (
      ...
      {/* Footer Actions */}
      <div className={`p-3 border-t bg-slate-50/50 shrink-0 z-20 ${isOver ? 'border-blue-200' : 'border-slate-100'} no-print`}>
        <button 
