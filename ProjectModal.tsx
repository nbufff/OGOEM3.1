                                    ...
                                    <div
                                        className={`absolute h-6 top-1/2 -translate-y-1/2 rounded shadow-sm border border-white/20 flex items-center justify-center overflow-hidden select-none group cursor-pointer
                                            ${isWP 
                                                ...
                                                : (data as ScheduledTask).isOTD 
                                                    ? 'bg-sky-300 ring-1 ring-sky-400' 
                                                    : (data as ScheduledTask).isCritical ? 'bg-red-500' : 'bg-blue-500'
                                            }
                                        `}
                                        style={{ left: barLeft, width: barWidth }}
                                        title={`${data.name}${(data as ScheduledTask).isOTD ? ' (OTD)' : ''}\nDuration: ${duration} days\n${dateRange}`}
                                    >
                                        {/* Show duration label if width allows */}
                                        {barWidth > 20 && (
                                            <div className="flex items-center gap-1 px-1">
                                                {(data as ScheduledTask).isOTD && (
                                                    <span className="text-[8px] bg-white text-sky-600 font-bold px-0.5 rounded leading-none">OTD</span>
                                                )}
                                                <span className={`text-[10px] font-medium drop-shadow-md truncate ${(data as ScheduledTask).isOTD ? 'text-sky-900' : 'text-white'}`}>
                                                    {duration}d
                                                </span>
                                            </div>
                                        )}
